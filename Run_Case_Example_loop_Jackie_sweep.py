...
#    if knob== 'nopgp':
#        tech_list[to_PGP_idx].remove()
#        tech_list[storage_PGP_idx].remove()
#        tech_list[from_PGP_idx].remove()
        
        
    if knob1 == 'tank': #tanks
        tech_list[storage_PGP_idx]['fixed_cost'] = 1.61524E-04 #($15/kWh)
        tech_list[storage_PGP_idx]['decay_rate'] = 1.14E-08 #(0.01%/year)
        
    if knob1 == 'salt': #salt cavern #BASE CASE
        tech_list[storage_PGP_idx]['fixed_cost'] = 3.73E-06 #($0.16/kWh)
        tech_list[storage_PGP_idx]['decay_rate'] = 1.14E-08 #(0.01%/year)
    
    if knob1 == 'dpr': #depleted reservior
        tech_list[storage_PGP_idx]['fixed_cost'] = 1.83865E-08 # ($0.002/kWh)
        tech_list[storage_PGP_idx]['decay_rate'] = 3.996133068895347e-08 #(0.035%/year)
        
    if knob2 == 'mcfc': #molten carbonate fuel cell
        tech_list[from_PGP_idx]['fixed_cost'] = 0.058464
        tech_list[from_PGP_idx]['efficiency'] = 0.7
        
    if knob2 == 'pemfc': #PEM fuel cell #BASE CASE
        tech_list[from_PGP_idx]['fixed_cost'] = 0.023461168 #($2000/kWh)
        tech_list[from_PGP_idx]['efficiency'] = 0.8
    
    if knob2 == 'h2tur': #hydrogen turbine
        tech_list[from_PGP_idx]['fixed_cost'] = 1.07683E-02 #($1000/kWh)
    ...
    def run_sweep(val):
        ...
        case_dic['case_name'] = case_name_default + '_'+ 'ng_' + knob1 + '_'+ knob2 + '_' + str(val)+ '__'

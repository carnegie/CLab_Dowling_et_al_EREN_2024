...
def run_3techs_1var(tech1, tech2, tech3, cost, eff, costrange1, costrange2, costrange3, effrange, case_name_orig, output_path_orig):

    case_name_default = case_name_orig
    output_path_default = output_path_orig
    case_dic['output_path'] = output_path_default + '/' + '2d_fixedeff'+ str(effrange[0]*effrange[0])
    # case_dic['output_path'] = output_path_default + '/' + tech1 + tech2 + tech3 + cost + eff
    
    knob1_idx = [i for i, j in enumerate(tech_list) if tech_list[i]['tech_name'] == tech1][0]
    knob2_idx = [i for i, j in enumerate(tech_list) if tech_list[i]['tech_name'] == tech2][0]
    knob3_idx = [i for i, j in enumerate(tech_list) if tech_list[i]['tech_name'] == tech3][0]
    
    tech_list[knob1_idx][eff] = effrange[0]
    tech_list[knob3_idx][eff] = effrange[0]
    ...
    if len(costrange1) == len(costrange2):
        ...
        for i in range(0, len(costrange1)):
            x1 = costrange1[i] #for labeling purposes
            x3 = costrange3[i] #for labeling purposes
            tech_list[knob1_idx][cost] = costrange1[i]
            tech_list[knob3_idx][cost] = costrange3[i]
            print('conversion cost status:', i, '/', len(costrange1))
            
            for j in range(0, len(costrange2)):
                x2 = costrange2[j] #for labeling purposes
                tech_list[knob2_idx][cost] = costrange2[j]
                # x2 = effrange[j] #for labeling purposes
                # tech_list[knob1_idx][eff] = effrange[j]
                # tech_list[knob3_idx][eff] = effrange[j]
                print('storage cost status:', j, '/', len(costrange2))
                case_dic['case_name'] = case_name_default + '_2d_' + tech1 + tech3 + '_' + cost + '_' + str(x1+x3) + '__' + tech2 + '_' + cost + '_' + str(x2) 
                print(case_dic['case_name'])
                print('rt efficiency:', tech_list[knob1_idx][eff]*tech_list[knob3_idx][eff])
                print('X storage cost:', costrange2[j])
                print('Y conversion costs:', costrange1[i]+costrange3[i])
...
# pcts = [0, 0.01, 0.1, 0.25, 0.5, 0.75, 0.9, 1, 3, 7.5, 10, 25, 50]
...
run_3techs_1var('to_PGP', 'PGP_storage', 'from_PGP', 'fixed_cost', 'efficiency', costrange1, costrange2, costrange3, effrange, case_name_orig, output_path_orig)           

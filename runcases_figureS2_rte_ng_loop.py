...
#Two knobs for RTE   
def run_condition(knob, idx1, idx2, var, therange, emissions, case_name_orig, output_path_orig):
    case_name_default = case_name_orig
    output_path_default = output_path_orig
    ...
    if case_dic.get('co2_constraint',-1) >= 0:
        ...
        case_dic['co2_constraint'] = emissions
    ...
    case_dic['output_path'] = output_path_default + '/' + knob + '_ng' + str(emissions)
    for val in therange:
    #set this index for correct tech and knob!
...
# rte = [1e-6] + [.1/100] + [1/100] + [.36] +list(np.linspace(5/100, 100/100, 20)) + [99/100]
rte = [1e-6] + [.1/100]  + [1/100] + [36/100] + list(np.linspace(5/100, 100/100, 20)) + [99/100]
therange = np.sqrt(rte)
#therange = [.6]
print(therange)
#4038.36*.05
run_condition('rtePGPeff_ng0.05', to_PGP_idx, from_PGP_idx,'efficiency',  therange, 201.918, case_name_orig, output_path_orig)
